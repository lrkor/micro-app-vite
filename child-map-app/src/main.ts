...
function handleMicroData(router: Router) {
    ...
    if (window.eventCenterForAppNameVite) {
        // 主动获取基座下发的数据
        console.log('child-map getData:', window.eventCenterForAppNameVite.getData());
        ...
        window.eventCenterForAppNameVite.addDataListener((data: Record<string, unknown>) => {
            console.log('监听数据更新');
            console.log('child-map addDataListener:', data);
        ...
        // 向基座发送数据
        setTimeout(() => {
            window.eventCenterForAppNameVite.dispatch({myname: 'child-map'});
        }, 3000);
    }
}
